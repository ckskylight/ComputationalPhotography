...
float Image::smartAccessor(int x, int y, int z, bool clamp) const{
    ...
    bool conditions[4];
    conditions[0] = x < 0;
    conditions[1] = x >= width();
    conditions[2] = y < 0;
    conditions[3] = y >= height();

    if (conditions[0] || conditions[1] || conditions[2] || conditions[3]) {
        if (clamp) {
            int new_x = x;
            int new_y = y;
            if (conditions[0]) new_x = 0;
            if (conditions[1]) new_x = width() - 1;
            if (conditions[2]) new_y = 0;
            if (conditions[3]) new_y = height() - 1;

            return operator()(new_x,new_y,z);
        }
        else {
            return 0;
        }
